...
4. Provider 表单定义、配置 parser、支付适配器和回调协议由代码注册表管理。
...
8. 新增 Provider 只增加注册定义和适配器代码，不修改数据库 schema，不执行 SQL migration，不新增专用 Vue 表单。
...
1. D1 保存真实值；
2. Provider parser 在服务端读取真实值；
3. 管理读取接口只返回 `{ configured: true, masked: "••••••••" }`；
4. 前端不接收 `configJson` 和敏感原文；
5. 未修改时提交 `keepExisting`；
6. 替换时提交新值；
7. 清除必须显式提交 `clear`；
8. 保存时服务端读取旧配置并合并，绝不把掩码写回 D1。
...
程序中只能有一个 `PaymentFlowService`。Provider Adapter 不能创建本地订单、修改订单、交付商品、发送推送或写支付日志。
...
6. 支付状态提交后触发消息推送和交付；
7. 交付失败记录 `DELIVERY_FAILED`，订单保持已付款并进入可重试交付状态。

支付已被第三方确认后，不能因为邮件或交付失败返回“支付失败”。支付结果和后置业务结果必须分开。
...
请求解析属于 Provider 协议，订单核对和状态更新属于统一回调服务，两者不能混合。
...
Adapter 的 `isValid=true` 只表示签名或加密协议有效，不代表订单可以直接标记已付款。
...
- 已付款但未交付：重复回调可触发幂等交付重试；
- 已交付：禁止再次分配卡密或写第二份成功交付记录。
...
onSavePaymentProvider({ provider, name, values, secrets })
...
- [x] **3.7 接入支付后置流程**：统一触发推送和幂等交付，交付失败不回滚已确认的付款状态。
...
- [-] **8.3 完成回调与并发测试**：已覆盖 Epay、BEpusdt 无效签名拒绝与 Stripe 查询订单归属；仍需以 D1 集成测试覆盖五个渠道的有效签名、商户、金额、币种、重复/并发回调、关闭订单和幂等交付。
