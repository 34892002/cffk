...
`notifyUrl` 和 `returnUrl` 都是 JSON definition 中的必填 URL 字段。`notifyUrl` 的 origin 取自站点设置，pathname 必须精确匹配注册表为该 Provider 声明的 Hono 回调端点，并且不能附带 query 或 hash。后台只展示和复制该固定地址，不允许编辑为不存在的子路径。`returnUrl` 可以在同源站点内调整。后台保存时从站点地址补齐二者；旧数据或手工写入的空 URL 会被严格 parser 判为无效配置，不能进入 Adapter。
