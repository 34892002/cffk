...
当前阶段只实现电子邮件的实际投递，但页面、数据库和服务层需要为微信（三方 API）、Telegram 保留清晰的扩展边界。
...
- `/push/history` 推送历史页；
- `server/email/service.ts` 邮件发送、日志和重试逻辑；
- `server/push/admin.telefunc.ts` 推送开关和推送日志查询；
- `pushConfig`、`pushLog`、`emailProvider`、`emailTemplate`、`emailLog`、`emailRetry` 数据表。
...
server/push/
  ...
  adapters/
    ...
    wechat.ts              # 微信第三方 API 适配器，第一阶段返回未接入
    telegram.ts            # Telegram 适配器，第一阶段返回未接入
  retry.ts                 # 统一重试调度；或暂时封装现有 emailRetry
  admin.telefunc.ts        # 后台配置和日志查询

server/email/
  service.ts               # 第一阶段可保留，但只作为 EMAIL adapter 内部实现
  order-events.ts          # 迁移后只负责组装业务事件，不直接发送邮件
```
...
第一阶段为了兼容当前 `pushConfig`，可以继续保留现有字段，在服务层转换成上述策略；后续再迁移为 JSON 策略或独立策略表。不要让新代码继续扩展 `emailEnabled`、`wecomEnabled`、`telegramEnabled` 与场景布尔字段的乘积。
...
- 只有 `contactType = EMAIL` 且邮箱格式有效时，才创建邮件客户投递任务。
...
当前阶段只允许勾选邮件。若客户联系方式不是邮箱，页面不需要展示额外渠道，因为服务端会跳过无法投递的任务。
...
- Telegram：未接入时禁用；存在已启用且配置有效的 Provider 时可选。
...
- `/push/telegram`：Telegram Bot Provider 配置和测试。
...
建议将现有 `emailProvider` 逐步抽象为通用的 `pushChannelConfig`。公共字段用于列表、筛选、启用和路由，Provider 专属字段全部放在 `configJson`：
...
3. 密钥字段只返回 `configured` 和掩码，不返回原文。
4. 用户未修改密钥时，提交值应为 `undefined` 或 `keepExisting: true`，不能把掩码保存回配置。
5. 用户清空密钥时必须显式提交 `clearSecret: true`，避免普通空字符串造成歧义。
6. 用户填写新密钥时，服务端替换 `configJson` 中对应的旧值。
7. 服务端根据 Provider schema 校验类型、必填项、范围、URL、邮箱和互斥字段。
8. 保存成功后返回新的脱敏值和配置状态，前端用返回结果刷新表单。
...
interface SaveProviderConfigInput {
  ...
  secrets?: Record<string, { value?: string; keepExisting?: boolean; clear?: boolean }>;
}
```
...
- 实施统一模型时，将 `emailProvider` 迁移为 `pushChannelConfig`；这是需要执行的一次 SQL migration。
...
- `emailLog` 和 `emailRetry` 在统一模型完成后分别由 `pushLog` 和 `pushRetry` 替代。
- 当前 `pushConfig` 的渠道与场景布尔字段仅用于迁移兼容，新代码不要继续为 Provider 增加布尔列。
...
2. 将 `server/email/service.ts` 中的开关判断、日志写入和重试封装到统一服务可调用的接口中。
3. 保留现有邮件 Provider、模板和测试页面，邮件适配器内部继续复用它们。
4. 将订单事件改成调用 `pushService.dispatch`，不再直接调用 `sendEmail`。
5. 统一历史优先读取 `pushLog`，邮件日志作为邮件专用详情或兼容数据。
...
- 对外部 API 设置超时、限流和最大响应体大小，避免 Worker 长时间占用。
...
5. D1 中旧的 `emailLog` 和新的统一日志是否需要长期双写？建议仅在迁移期双写，统一历史稳定后停止双写。
