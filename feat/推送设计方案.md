...
- `pushChannelConfig`、`pushConfig`、`pushPolicy`、`pushLog`、`pushRetry`、`emailTemplate` 数据表。
...
策略已由独立的 `pushPolicy` 表保存。`pushConfig` 只保留全局推送开关；不要重新增加 `emailEnabled`、`wecomEnabled`、`telegramEnabled` 或场景布尔字段。
...
- `pushConfig` 只保存全局开关，场景和渠道策略统一由 `pushPolicy` 保存。
