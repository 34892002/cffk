...
async function dispatchEvent(database: D1Database, runtime: EmailRuntime, event: typeof orderEvent.$inferSelect) {
  ...
  if (event.scene === "PAYMENT_EXCEPTION") {
    await dispatchPush(database, runtime, { scene: event.scene, messageType: "ADMIN", orderId: event.orderId, variables: payload, source });
    return;
  }
  await Promise.all([
...
export async function processOrderEvents(database: D1Database, runtime: EmailRuntime, now = new Date(), limit = 50) {
  ...
  for (const event of candidates) {
    ...
    if (!claimed) continue;
    try {
      await dispatchEvent(database, runtime, event);
  ...
  return { attempted: candidates.length, processed };
}
