...
async function decryptHashpay(rawBody: string, privateKeyPem: string) {
  const envelope = JSON.parse(rawBody) as { key: string; iv: string; data: string; alg?: string };
  if (!envelope.key || !envelope.iv || !envelope.data) throw new Error("HASHPAY_CALLBACK_INVALID");
  ...
  return JSON.parse(new TextDecoder().decode(plaintext)) as { timestamp: number; payload: Record<string, unknown> };
}
