...
export class PaymentFlowService {
  ...
  async resume(orderNo: string, ownerUserId: string | null, email?: string): Promise<PaymentCreateResult> {
    ...
    if (record.deliveryType === "CARD_AUTO") {
      const [stock] = await db.select({ available: count() }).from(card).where(and(record.productSkuId ? eq(card.productSkuId, record.productSkuId) : eq(card.productId, record.productId), eq(card.status, "UNUSED")));
      ...
      const [sku] = record.productSkuId ? await db.select({ physicalStock: productSku.physicalStock }).from(productSku).where(eq(productSku.id, record.productSkuId)).limit(1) : [];
