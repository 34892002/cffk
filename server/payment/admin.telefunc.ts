...
import { paymentLog, paymentProvider } from "@/database/drizzle/schema";
import { appError } from "@/lib/app-error";
import { getProviderDefinition, paymentProviderDefinitions, parseProviderConfig, type PaymentProviderKind } from "./registry";
...
function getSafeForm(provider: string, name: string, isEnabled: boolean, sort: number, configJson: string, updatedAt?: Date) {
  ...
  for (const field of definition.fields) {
    const value = parsed[field.key] ?? definition.defaults[field.key];
    if (field.secret) secrets[field.key] = maskedSecret(value);
    else if (value !== undefined) values[field.key] = value as JsonValue;
  }
  return { provider, title: definition.title, name, isEnabled, sort, updatedAt: updatedAt?.toISOString() ?? null, valid, schemaVersion: definition.schemaVersion, fields: definition.fields, values, secrets };
}
...
export async function onGetPaymentProviders() {
  ...
  return (Object.keys(paymentProviderDefinitions) as PaymentProviderKind[]).map((provider, index) => {
    ...
    return getSafeForm(provider, record?.name ?? paymentProviderDefinitions[provider].title, record?.isEnabled ?? false, record?.sort ?? (index + 1) * 10, record?.configJson ?? JSON.stringify(paymentProviderDefinitions[provider].defaults), record?.updatedAt)!;
  });
}
...
export async function onGetPaymentProviderForm(input: { provider: PaymentProviderKind }) {
  ...
  return getSafeForm(input.provider, record?.name ?? paymentProviderDefinitions[input.provider].title, record?.isEnabled ?? false, record?.sort ?? 0, record?.configJson ?? JSON.stringify(paymentProviderDefinitions[input.provider].defaults), record?.updatedAt);
}
...
export async function onSavePaymentProvider(input: {
  ...
  const [current] = await db.select().from(paymentProvider).where(eq(paymentProvider.provider, input.provider)).limit(1);
  const configJson = mergePaymentProviderConfig({ provider: input.provider, currentConfigJson: current?.configJson, values: input.values, secretUpdates: input.secretUpdates });
  const timestamp = now();
  if (current) {
...
export async function onValidatePaymentProviderConfig(input: {
  ...
  mergePaymentProviderConfig({ provider: input.provider, currentConfigJson: record.configJson, values: input.values, secretUpdates: input.secretUpdates });
