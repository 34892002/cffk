...
import { automaticDeliveryJob, customerAddress, discountCode, order, orderDelivery, product, productSku } from "@/database/drizzle/schema";
...
export async function createOrder(database: D1Database, input: CreateOrderInput, ownerUserId: string | null): Promise<CreatedOrder> {
  ...
  const sku = input.productSkuId ? (await db.select().from(productSku).where(and(eq(productSku.id, positiveInteger(input.productSkuId, "PRODUCT_SKU_ID")), eq(productSku.productId, item.id), eq(productSku.status, "ACTIVE"))).limit(1))[0] : await getDefaultProductSku(db, item.id);
  if (!sku) fail("PRODUCT_SKU_NOT_AVAILABLE");
  ...
  if (sku.deliveryType === "CARD_AUTO" && (await countAvailableCards(database, item.id, sku.id)) < quantity) fail("PRODUCT_STOCK_NOT_ENOUGH");
...
export async function deliverPaidOrder(database: D1Database, orderId: number): Promise<DeliveryResult> {
  ...
  try {
    const contents = record.deliveryType === "FIXED_CARD"
      ...
      : (await allocateCardsForPaidOrder(database, orderId, record.productId, record.quantity, record.productSkuId)).map((item) => item.content);
