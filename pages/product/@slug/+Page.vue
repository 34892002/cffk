      ...
      <aside class="h-fit lg:sticky lg:top-6">
        <Card>
          <CardHeader><CardDescription>当前价格</CardDescription><CardTitle class="text-3xl">¥{{ data.price }}</CardTitle></CardHeader><form class="grid gap-6" @submit.prevent="onSubmit">
            ...
            </CardContent><CardFooter class="flex-col items-stretch gap-3"><Button type="submit" :disabled="loading || isOutOfStock || (requiresPayment && !methods.length)">{{ loading ? "处理中..." : isOutOfStock ? "暂时缺货" : requiresPayment ? "提交订单并支付" : "创建零元订单" }}</Button><p class="text-xs text-muted-foreground">支付金额与订单状态均由服务端校验。</p></CardFooter>
          </form>
        </Card>
      </aside>
    </section>
