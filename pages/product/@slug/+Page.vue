          ...
          <Card>
            <CardHeader><CardDescription>{{ messages.productCheckout.currentPrice }}</CardDescription><CardTitle class="text-3xl text-orange-600">¥{{ selectedSku.price }}</CardTitle></CardHeader><form class="grid min-w-0 gap-6" novalidate @submit.prevent="submit">
              <CardContent class="grid min-w-0 gap-4">
                <label v-if="data.skus.length > 1" class="grid gap-2 text-sm font-medium"><span>选择规格</span><Select v-model="selectedSkuId"><SelectTrigger><SelectValue /></SelectTrigger><SelectContent><SelectItem v-for="sku in data.skus" :key="sku.id" :value="String(sku.id)">{{ sku.name }} · ¥{{ sku.price }}</SelectItem></SelectContent></Select></label>
...
watch(selectedSku, (sku) => { if (sku) quantity.value = sku.minBuy; }); const contactValue = ref(user?.email?.trim() ?? ""); const discountCode = ref(""); const buyerNote = ref(""); const discountApplying = ref(false); type DiscountPreview = Awaited<ReturnType<typeof onPreviewDiscount>>; const discountPreview = ref<DiscountPreview | null>(null); const loading = ref(false);
