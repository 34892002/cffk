    ...
    <Card v-for="item in providers" :key="item.provider">
      ...
      <form @submit.prevent="saveProvider(item.provider)">
        <CardContent class="grid max-w-3xl gap-4">
          ...
          <label class="flex items-center justify-between gap-3 text-sm font-medium"><span>启用此渠道</span><Switch v-model="forms[item.provider].isEnabled" /></label>
...
async function loadProviders() {
  ...
  error.value = null;
  try {
    providers.value = await runTelefunc(() => onGetPaymentProviders(), { notifyError: false });
...
async function saveProvider(provider: Provider["provider"]) {
  ...
  error.value = null;
  try {
    await runTelefunc(() => onSavePaymentProvider({ provider, ...form }), { notifyError: false });
    await loadProviders();
  } catch (cause) {
    error.value = userErrorMessage(cause);
  } finally {
