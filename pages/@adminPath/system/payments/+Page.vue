<template>
  <PaymentSettingsLayout>
    ...
    <Dialog v-model:open="dialogOpen"><DialogContent class="grid max-h-[calc(100dvh-2rem)] w-[calc(100%-2rem)] max-w-3xl grid-rows-[auto_minmax(0,1fr)] overflow-hidden p-0" @interact-outside.prevent @escape-key-down.prevent><DialogHeader class="border-b px-6 py-5 pr-14"><DialogTitle>配置{{ editing?.title ?? "支付渠道" }}</DialogTitle><DialogDescription>配置直接保存在 D1。敏感字段编辑时不会回显，留空将保留现有值。</DialogDescription></DialogHeader><form v-if="editing" class="grid min-h-0 grid-rows-[minmax(0,1fr)_auto]" novalidate @submit.prevent="saveProvider"><div class="overflow-y-auto px-6 py-5"><FieldSet class="gap-4"><FieldLegend>基础设置</FieldLegend><div class="grid gap-4 sm:grid-cols-[minmax(0,1fr)_auto] sm:items-end"><Field><FieldLabel for="payment-provider-name">渠道名称</FieldLabel><Input id="payment-provider-name" v-model="form.name" autocomplete="off" /></Field><Field orientation="horizontal" class="h-9 whitespace-nowrap sm:w-32"><FieldLabel for="payment-provider-enabled">启用渠道</FieldLabel><Switch id="payment-provider-enabled" v-model:checked="form.isEnabled" /></Field></div></FieldSet><FieldSet class="mt-6 gap-4"><FieldLegend>连接参数</FieldLegend><div class="grid gap-y-5"><JsonFormFields :fields="editing.fields" :values="form.values" :secrets="form.secrets" :errors="fieldErrors" @update:values="form.values = $event" /><PaymentUrlFields :provider="editing.provider" :show-notify="hasNotifyUrl" :notify-url="urlValue('notifyUrl')" :return-url="urlValue('returnUrl')" :site-url="editing.siteUrl" @update:notify-url="form.values.notifyUrl = $event" @update:return-url="form.values.returnUrl = $event" /></div></FieldSet><FieldSet v-if="configuredSecretFields.length" class="mt-6 gap-3"><FieldLegend>敏感配置</FieldLegend><FieldDescription>勾选后将在保存时清除对应的 D1 配置。</FieldDescription><div class="grid gap-3 sm:grid-cols-2"><Field v-for="field in configuredSecretFields" :key="field.key" orientation="horizontal"><Checkbox :id="`clear-${field.key}`" v-model="form.clearSecrets[field.key]" /><FieldLabel :for="`clear-${field.key}`">清除 {{ field.label }}</FieldLabel></Field></div></FieldSet></div><DialogFooter class="border-t bg-background px-6 py-4"><Button type="button" variant="outline" :disabled="saving" @click="testProvider">校验配置</Button><DialogClose as-child><Button type="button" variant="outline" :disabled="saving">取消</Button></DialogClose><Button type="submit" :disabled="saving">{{ saving ? "保存中..." : "保存配置" }}</Button></DialogFooter></form></DialogContent></Dialog>
  </PaymentSettingsLayout>
</template>
...
const configuredSecretFields = computed(() => editing.value?.fields.filter((field) => field.secret && form.secrets[field.key]?.configured) ?? []); const hasNotifyUrl = computed(() => editing.value?.fields.some((field) => field.key === "notifyUrl") ?? false); const siteUrlMissing = computed(() => loaded.value && !providers.value[0]?.siteUrl);
