    ...
    <AdminDataTable :columns="columns" :rows="paginatedDiscounts" row-key="id">
      <template #toolbar>
        <div class="flex flex-wrap items-center gap-2">
          <Input v-model="draftQuery" class="h-8 w-60" placeholder="搜索优惠码" @keyup.enter="search" />
    ...
    <Dialog v-model:open="dialogOpen">
      <DialogContent class="grid max-h-[calc(100vh-2rem)] w-[calc(100%-2rem)] max-w-xl grid-rows-[auto_minmax(0,1fr)] overflow-hidden p-0">
        ...
        <form class="grid min-h-0 grid-rows-[minmax(0,1fr)_auto]" @submit.prevent="saveDiscount">
          <div class="min-h-0 overflow-y-auto px-6 py-5"><div class="grid gap-4"><label class="grid gap-2 text-sm font-medium">优惠码<Input v-model="form.code" required maxlength="64" placeholder="SUMMER2026" /></label><label class="grid gap-2 text-sm font-medium">类型<Select v-model="form.type"><SelectTrigger class="h-9"><SelectValue /></SelectTrigger><SelectContent><SelectItem value="FIXED">固定减免（元）</SelectItem><SelectItem value="PERCENT">百分比减免</SelectItem></SelectContent></Select></label><label class="grid gap-2 text-sm font-medium">{{ form.type === "FIXED" ? "优惠金额（元）" : "优惠百分比" }}<Input v-model="form.value" inputmode="decimal" :min="form.type === 'PERCENT' ? 1 : '0.01'" :max="form.type === 'PERCENT' ? 100 : undefined" required /></label><label class="grid gap-2 text-sm font-medium">最低订单金额（元）<Input v-model="form.minAmount" inputmode="decimal" placeholder="0 表示不限" /></label><label class="grid gap-2 text-sm font-medium">最多使用次数<Input v-model.number="form.maxUses" type="number" min="0" /></label><label class="grid gap-2 text-sm font-medium">适用商品 ID<Input v-model="form.productIds" class="h-9 rounded-md border border-input bg-background px-3 py-2 text-sm" placeholder="例如：1,2,3；留空为全部商品" /></label><label class="grid gap-2 text-sm font-medium">过期时间<Input v-model="form.expiresAt" type="datetime-local" class="h-9 rounded-md border border-input bg-background px-3 py-2 text-sm" /></label><label class="flex items-center justify-between gap-3 text-sm font-medium"><span>启用此优惠码</span><Switch v-model="form.isActive" /></label></div></div>
...
function editDiscount(item: Discount) { Object.assign(form, { id: item.id, code: item.code, type: item.type, value: item.value, minAmount: item.minAmount ?? "", maxUses: item.maxUses ?? 0, productIds: item.productIds ?? "", expiresAt: item.expiresAt ? toLocalDateTime(item.expiresAt) : "", isActive: item.isActive }); dialogOpen.value = true; }
