...
import { onDeleteProduct, onGetCatalogAdminData, onSaveProduct, onSetProductStatus } from "@/server/catalog/admin.telefunc";
...
const catalog = reactive<Catalog>({ items: [], total: 0, page: 1, pageSize: 10, categories: [] }); const filters = reactive<{ keyword?: string; categoryId?: number; status?: ProductStatus }>({}); const draftKeyword = ref(""); const draftCategoryId = ref<number | undefined>(); const loading = ref(false); const saving = ref(false); const deleting = ref(false); const error = ref<string | null>(null); const catalogLoaded = ref(false); const dialogOpen = ref(false); const deleteDialogOpen = ref(false); const productToDelete = ref<Product | null>(null);
