  ...
  <section class="flex w-full flex-col gap-6">
    ...
    <Dialog v-model:open="detailOpen">
      <DialogContent class="grid max-h-[calc(100dvh-2rem)] w-[calc(100%-2rem)] max-w-3xl grid-rows-[auto_minmax(0,1fr)] overflow-hidden p-0">
        ...
        <div v-if="detail" class="grid min-h-0 gap-6 overflow-y-auto px-6 py-5 lg:grid-cols-2"><div class="grid content-start gap-3 text-sm"><p><span class="text-muted-foreground">商品：</span>{{ detail.order.productNameSnapshot }}</p><p><span class="text-muted-foreground">联系方式：</span>{{ detail.order.contactType }} / {{ detail.order.contactValue || "-" }}</p><p><span class="text-muted-foreground">收件信息：</span>{{ detail.order.receiverInfo || "-" }}</p><p><span class="text-muted-foreground">备注：</span>{{ detail.order.buyerNote || "-" }}</p><p><span class="text-muted-foreground">优惠码：</span>{{ detail.order.discountCodeStr || "-" }}</p></div><div class="grid content-start gap-5"><div><p class="text-sm font-medium">发货快照</p><pre class="mt-2 max-h-44 overflow-auto whitespace-pre-wrap rounded-md border bg-muted/40 p-3 text-xs">{{ detail.deliveries.length ? detail.deliveries.map((item) => item.contentSnapshot).join("\n") : "暂无成功发货快照" }}</pre></div><div><p class="text-sm font-medium">支付日志</p><div class="mt-2 grid gap-2 text-xs text-muted-foreground"><p v-for="item in detail.payments" :key="item.id">{{ formatDate(item.createdAt) }} · {{ item.verifyStatus }} · {{ item.message || item.eventType }}</p><p v-if="!detail.payments.length">暂无支付日志</p></div></div></div></div>
      </DialogContent>
    </Dialog>
...
const columns: AdminTableColumn<Order>[] = [
  { key: "orderNo", label: "订单" }, { key: "productName", label: "商品" }, { key: "quantity", label: "数量" }, { key: "contactValue", label: "联系方式" }, { key: "amount", label: "金额" }, { key: "payment", label: "支付" }, { key: "delivery", label: "发货" }, { key: "createdAt", label: "创建时间" },
];
